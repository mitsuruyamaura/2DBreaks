...
public class MoviePopup : MonoBehaviour {
    ...
    private void ProcPlayMovies(CancellationToken token, bool isWaitTapBetweenVideos) {
        PlayMoviesAsync(token, isWaitTapBetweenVideos).Forget();
    }
    ...
    /// <param name="isWaitTapBetweenVideos"></param>
    /// <returns></returns>
    private async UniTask PlayMoviesAsync(CancellationToken token, bool isWaitTapBetweenVideos) {
        ...
        await playlistPlayer.PlayAsync(playlistView.GetPlayListDataFromDetail(), token, isWaitTapBetweenVideos);
    ...
    public virtual void AddPlaylist(MovieDetailView movieDetailView, int slotIndex = -1) {
        ...
        playlistDetailView.SetupPlaylistDetailView(movieDetailView.VideoData, RemoveFromCurrnetTeam);
    ...
    public virtual void RemoveFromCurrnetTeam(PlaylistDetailView playlistDetailView) {
